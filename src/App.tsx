...
function App() {
  ...
  const addToCard = useCallback((e: FormEvent<any>) => {
    e.preventDefault()
  ...
  return (
    <div>
      <div className="app-header">
        ...
        <button className="btn absolute right-4" onClick={() => { window.my_modal_3.showModal(); setIsNewItem(false); }}>
      ...
      <Modal header="Cart" results={[
          ...
          <Fragment key={index}>
            <div className="flex justify-between">
              ...
              <div className="text-xl flex items-center gap-4">
                <button type="button" onClick={decrementAmount} value={id} className="btn btn-primary btn-sm">{'-'}</button>
                <p>{amount}</p>
                <button type="button" onClick={incrementAmount} value={id} className="btn btn-primary btn-sm">{'+'}</button>
              </div>
            </div>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={applyDiscount} className="btn">Apply</button>
        </div>
      </Modal>
